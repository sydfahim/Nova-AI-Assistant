...
/** Secure proxy that forwards /api/chat to OpenAI — API key never reaches the browser. */
function novaApiPlugin(): Plugin {
  return {
    name: 'nova-api',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use('/api/chat', async (req: any, res: any) => {
        ...
        if (!apiKey) {
          res.statusCode = 500
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ error: 'OPENAI_API_KEY is not set. Add it to your environment secrets.' }))
          return
        }
        ...
        try {
              ...
              messages: [
                {
                  role: 'system',
                  content:
                    'You are Nova, a premium personal AI assistant. Be thoughtful, concise, and precise. Use markdown to structure your responses when it adds clarity.',
                },
                ...messages,
              ],
            }),
          ...
          if (!upstream.ok) {
            ...
            res.write(`data: ${JSON.stringify({ error: `OpenAI ${upstream.status}: ${errText}` })}\n\n`)
            res.end()
            return
          }
